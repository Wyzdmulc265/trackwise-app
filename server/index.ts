...
for (const [key, cfg] of Object.entries(requiredEnvVars)) {
  ...
  if (!value) {
    if ((cfg as any).optional) continue;
  ...
  if ((cfg as any).minLen && value.length < (cfg as any).minLen) {
    console.error(`FATAL: ${key} is too short. Must be at least ${(cfg as any).minLen} characters.`);
    process.exit(1);
  }
}
