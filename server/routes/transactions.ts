...
router.get('/', authenticate, requireTenant, async (req: Request, res: Response) => {
  try {
    ...
    const transactions = await query<any>(querySQL, params);

    let countSQL = `SELECT COUNT(*) FROM transactions WHERE tenant_id = $1`;
    const countParams: unknown[] = [businessKey];
    if (type) {
    ...
    res.json({
      transactions,
      pagination: { total, limit: parseInt(limit as string) || 100, offset: parseInt(offset as string) || 0, hasMore: total > (parseInt(offset as string) || 0) + (parseInt(limit as string) || 100) },
    });
  } catch (error) {
...
router.get('/:txId', authenticate, requireTenant, async (req: Request, res: Response) => {
  try {
  ...
  try {
    ...
    if (itemId && quantity) {
      const item = await queryOne<any>(
        'SELECT id, quantity FROM inventory_items WHERE id = $1 AND tenant_id = $2',
        [itemId, businessKey]
      );
      if (!item) {
    ...
    if (user.role === 'Accountant') {
      ...
      await queryExecute(
        ...
        [approvalId, businessKey, 'transaction', 'create', JSON.stringify({ type, date, category, amount, description, itemId, quantity }), user.username, 'pending']
      );
    ...
    await withTransaction(async (client) => {
      await client.query(
        ...
        [txId, businessKey, type, date, category, amount, description, itemId || null, quantity || null, user.username]
      );
...
// ─── UPDATE transaction ───────────────────────────────────────────────────────
router.put('/:txId', authenticate, requireTenant, async (req: Request, res: Response) => {
  try {
    const user = (req as any).user as AuthPayload;
    const { businessKey } = req.params;
    ...
    if (user.role === 'Accountant') {
      ...
      await queryExecute(
        ...
        [approvalId, businessKey, 'transaction', 'update', JSON.stringify(updates), txId, JSON.stringify(existingTx), user.username, 'pending']
      );
    ...
    if (updates.type) { setClause.push(`type = $${values.length + 1}`); values.push(updates.type); }
    if (updates.date) { setClause.push(`date = $${values.length + 1}`); values.push(updates.date); }
    if (updates.category) { setClause.push(`category = $${values.length + 1}`); values.push(updates.category); }
    if (updates.amount !== undefined) { setClause.push(`amount = $${values.length + 1}`); values.push(updates.amount); }
    if (updates.description) { setClause.push(`description = $${values.length + 1}`); values.push(updates.description); }
    if (updates.itemId !== undefined) { setClause.push(`item_id = $${values.length + 1}`); values.push(updates.itemId || null); }
    if (updates.quantity !== undefined) { setClause.push(`quantity = $${values.length + 1}`); values.push(updates.quantity || null); }
    ...
    await withTransaction(async (client) => {
      await client.query(
        `UPDATE transactions SET ${setClause.join(', ')} WHERE id = $${values.length}`,
        values
      );
...
// ─── DELETE transaction ───────────────────────────────────────────────────────
router.delete('/:txId', authenticate, requireTenant, async (req: Request, res: Response) => {
  try {
    const user = (req as any).user as AuthPayload;
    const { businessKey } = req.params;
    const { txId } = req.params;
    ...
    if (user.role === 'Accountant') {
      ...
      await queryExecute(
        `INSERT INTO pending_approvals (id, tenant_id, kind, action, payload, target_id, target_snapshot, requested_by, requested_at, status)
         VALUES ($1, $2, $3, $4, $5, $6, $7, $8, NOW(), $9)`,
        [approvalId, businessKey, 'transaction', 'delete', JSON.stringify(null), txId, JSON.stringify(existingTx), user.username, 'pending']
      );
    ...
    await withTransaction(async (client) => {
      if (existingTx.item_id && existingTx.quantity) {
        if (existingTx.type === 'sale') {
          ...
          await client.query(
            `UPDATE inventory_items SET quantity = quantity - $1 WHERE id = $2`,
            [existingTx.quantity, existingTx.item_id]
          );
        }
      }
