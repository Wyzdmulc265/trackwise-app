...
router.get('/', authenticate, requireAdmin, requireTenant, async (req: Request, res: Response) => {
  try {
    ...
    const users = await query<any>(
      `SELECT id, tenant_id, owner_name, username, business_name, contact, role, must_change_password, created_at, created_by
...
router.post('/', authenticate, requireAdmin, requireTenant, async (req: Request, res: Response) => {
  try {
    ...
    const existing = await queryOne<{ username: string }>(
      'SELECT username FROM users WHERE tenant_id = $1 AND LOWER(username) = LOWER($2)',
...
router.put('/:userId', authenticate, requireAdmin, requireTenant, async (req: Request, res: Response) => {
  try {
...
router.delete('/:userId', authenticate, requireAdmin, requireTenant, async (req: Request, res: Response) => {
  try {
    const { userId } = req.params;
    const { businessKey } = req.params;
    ...
    const targetUser = await queryOne<{ id: string; role: string }>(
      'SELECT id, role FROM users WHERE id = $1 AND tenant_id = $2',
      [userId, businessKey]
    );
    if (!targetUser) {
...
router.post('/:userId/reset-password', requireAdmin, requireTenant, async (req: Request, res: Response) => {
  try {
    const { userId } = req.params;
