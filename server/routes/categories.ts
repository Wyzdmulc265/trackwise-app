...
import { z } from 'zod';
import { query, queryOne, queryExecute } from '../db/client';
...
router.get('/', authenticate, requireTenant, async (req: Request, res: Response) => {
  try {
    ...
    const categories = await query<any>(
      `SELECT id, tenant_id, name, type, is_custom, created_at, updated_at FROM categories WHERE tenant_id = $1 ORDER BY name`,
      [businessKey]
    );
...
// ─── GET by type ─────────────────────────────────────────────────────────────
router.get('/type/:type', authenticate, requireTenant, async (req: Request, res: Response) => {
  try {
    const { businessKey, type } = req.params;
    const categoryType = type as 'sale' | 'purchase' | 'expense';
    ...
    const categories = await query<any>(
      `SELECT id, tenant_id, name, type, is_custom, created_at, updated_at FROM categories WHERE tenant_id = $1 AND type = $2 ORDER BY name`,
      [businessKey, type]
    );
...
router.post('/', authenticate, requireAdmin, requireTenant, async (req: Request, res: Response) => {
  try {
    ...
    await queryExecute(
      `INSERT INTO categories (id, tenant_id, name, type, is_custom, created_at, updated_at) VALUES ($1, $2, $3, $4, $5, NOW(), NOW())`,
      [categoryId, businessKey, name, type, true]
    );

    const created = await queryOne<any>(
      `SELECT id, tenant_id, name, type, is_custom, created_at, updated_at FROM categories WHERE id = $1`,
      [categoryId]
    );
...
router.delete('/:categoryId', authenticate, requireAdmin, requireTenant, async (req: Request, res: Response) => {
  try {
