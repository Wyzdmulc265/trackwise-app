...
import { hashPassword, verifyPassword, generateTokens, storeRefreshToken, revokeRefreshToken, revokeAllUserTokens, verifyRefreshToken, } from '../utils/tokens.js';
...
const router = Router();
const logAction = (req, action, extra) => {
...
const handleJwtVerify = (req, next, token) => {
    try {
...
const validateBusinessKey = async (req) => {
    // The businessKey comes from the URL params in registration
    return req.params.businessKey;
};
...
router.post('/register', async (req, res, next) => {
    try {
        ...
        const existingUser = await queryOne('SELECT username FROM users WHERE LOWER(username) = LOWER($1)', [username]);
        if (existingUser) {
            throw new ValidationError('That username is already taken');
        }
        const existingBusiness = await queryOne('SELECT name FROM businesses WHERE LOWER(name) = LOWER($1)', [businessName]);
        if (existingBusiness) {
        ...
        await queryExecute(`INSERT INTO businesses (id, tenant_id, name, owner_name, contact) VALUES ($1, $2, $3, $4, $5)`, [businessId, businessKey, businessName, ownerName, contact]);
        const passwordHash = await hashPassword(password);
        const userId = withPrefix('usr');
        await queryExecute(`INSERT INTO users (id, tenant_id, owner_name, username, business_name, contact, password_hash, role, must_change_password, created_at)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9, NOW())`, [userId, businessKey, ownerName, username, businessName, contact, passwordHash, 'Admin', false]);
        ...
        for (const [name, type] of defaultCategories) {
            await queryExecute(`INSERT INTO categories (id, tenant_id, name, type, is_custom, created_at)
         VALUES ($1, $2, $3, $4, $5, NOW())`, [withPrefix('cat'), businessKey, name, type, true]);
        }
        const payload = {
            userId,
            username,
        ...
        if (error instanceof ValidationError)
            return next(error);
        if (error?.code === '23505')
            return next(new ValidationError('Duplicate data - username or business name already exists'));
        return next(error);
    }
});
// ─── Login ───────────────────────────────────────────────────────────────────
router.post('/login', async (req, res, next) => {
    try {
        ...
        const user = await queryOne(`SELECT id, username, business_name, owner_name, password_hash, role, must_change_password, tenant_id
       FROM users WHERE LOWER(username) = LOWER($1) AND LOWER(business_name) = LOWER($2)`, [username, businessName]);
        if (!user) {
...
router.get('/me', async (req, res, next) => {
    try {
        ...
        const user = await queryOne(`SELECT id, username, business_name, owner_name, role, must_change_password, tenant_id
...
router.post('/change-password', async (req, res, next) => {
    try {
        ...
        const user = await queryOne('SELECT id, password_hash FROM users WHERE id = $1', [payload.userId]);
        if (!user) {
            throw new ValidationError('User not found');
        }
        ...
        await queryExecute('UPDATE users SET password_hash = $1, updated_at = NOW() WHERE id = $2', [newPasswordHash, payload.userId]);
